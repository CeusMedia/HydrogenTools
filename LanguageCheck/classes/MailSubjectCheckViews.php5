<?php
class MailSubjectCheckViews
{
	static public function buildContent( $path, $project, $source, $target )
	{
		$sc			= new MailSubjectCheck( $path.$source."/", $path.$target."/" );
		$results	= $sc->getMailSubjectDifference();
		
		$errors	= max( $results['fileList'], $results['encodeList'], $results['translateList'] );
		if( !$errors )
		{
			$fileList	= "<b style='color: green'>All Mail Subject Components are OK</b>";
		}
		else
		{
		...
		return require_once( "templates/subjects.phpt" );
	}
	...
	static private function buildContentEncodingList( $path, $project, $source, $target, $resultList )
	{
		...
		foreach( $resultList as $fileName )
		{
			...
			$link		= UI_HTML_Elements::Link( "?project=".$project."&source=".$source."&target=".$target."&fileName=".$fileName."&action=encodeMailSubjectFile", $icon );
	...
	static private function buildContentFileList( $path, $project, $source, $target, $resultList )
	{
		...
		foreach( $resultList as $fileName )
		{
			...
			$link		= UI_HTML_Elements::Link( "?project=".$project."&source=".$source."&target=".$target."&fileName=".$fileName."&action=copyMailSubjectFile", $icon );
			$files[]	= UI_HTML_Elements::ListItem( $link.$fileName );
		}
		$list[]	= UI_HTML_Elements::unorderedList( $files );
		return implode( "", $list );
	...
	static private function buildContentToTranslate( $path, $project, $source, $target, $resultList )
	{
		...
		foreach( $resultList as $fileName )
		{
			...
			$link		= UI_HTML_Elements::Link( "javascript: editMailSubjectValue( '?project=".$project."&source=".$source."&target=".$target."&fileName=".$fileName."&action=editMailSubjectValue', '".urlencode( addslashes( $value ) )."' );", $icon );
